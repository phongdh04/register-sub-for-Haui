...
```bash
cd backend-queue
go build -o queue-service.exe main.go
./queue-service.exe
# Hoặc chạy trực tiếp:
# go run main.go
```

> **⚠️ LƯU Ý LỖI KHI CHẠY GO TRÊN WINDOWS:**
> Nếu bị lỗi: `error obtaining buildID for go tool asm... An Application Control policy has blocked this file` -> Lỗi này do môi trường Windows chặn file `.exe` (Vì thư mục dự án không nằm trong Program Files). 
> **Cách khắc phục:** 
> 1. Mở **Windows Security** -> Vô hiệu hoá tạm thời hoặc thêm Folder gốc của dự án vào vùng Exclusions (Loại trừ).
> 2. Hoặc mở Terminal bằng quyền **Run as Administrator**.
> 3. Hoặc **bỏ qua bước 4 này**, vì thực chất Go Queue đã được tích hợp & tự động chạy sẵn bên trong Bước 2 (Docker) rồi!

**Port hoạt động:** Go Service chạy ở port `3000`.
